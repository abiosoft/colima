...
- **Description**: Automatically set by Colima when Kubernetes is enabled. Points to the k3s configuration file.
- **Default**: Set by Colima when Kubernetes is active
- **Usage**:
  ```sh
  # Automatically set by Colima when --kubernetes is used
  export KUBECONFIG=$HOME/.colima/default/kubeconfig.yaml
  ```
...
See the [Lima documentation](https://github.com/lima-vm/lima) for more details on these advanced options.

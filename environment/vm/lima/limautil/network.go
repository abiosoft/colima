...
// IPAddress returns the ip addresses for profile.
// It returns the PTP addresses if networking is enabled or falls back to 127.0.0.1.
// It is guaranteed to return at least one value.
//
	...
	if len(instance.Network) > 0 {
		for _, n := range instance.Network {
			if n.Interface == NetInterface {
				addresses := getIPAddress(profileID, n.Interface)
				if len(addresses) > 0 {
					return addresses
				}
			}

...
func New(host environment.HostActions) environment.VM {
	env := limaInstanceEnvVar + "=" + config.Profile().Name
...
func limaConfDir() string {
	home := util.HomeDir()
	return filepath.Join(home, ".lima", config.Profile().Name)
}
...
func (l *limaVM) Start(conf config.Config) error {
	...
	configFile := config.Profile().Name + ".yaml"
...
func (l limaVM) resume(conf config.Config) error {
	...
	a.Stage("starting")
	a.Add(func() error {
		return l.host.Run(limactl, "start", config.Profile().Name)
	})
...
func (l limaVM) Stop() error {
	...
	a.Add(func() error {
		return l.host.Run(limactl, "stop", config.Profile().Name)
	})
...
func (l limaVM) Teardown() error {
	...
	a.Add(func() error {
		return l.host.Run(limactl, "delete", config.Profile().Name)
	})
